---
name: golang-testing
description: 包含表驱动测试、子测试、基准测试、模糊测试和测试覆盖率的 Go 测试模式。遵循 TDD 方法论并采用惯用的 Go 编程实践。
---

# Go 测试模式

全面的 Go 测试模式，用于编写可靠且易维护的测试，遵循 TDD 方法论。
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            ...
            if !reflect.DeepEqual(got, tt.want) {
                t.Errorf("得到 %+v; 期望 %+v", got, tt.want)
            }
        })
    }
}
...
func TestUser(t *testing.T) {
    ...
    t.Run("获取", func(t *testing.T) {
        user, err := db.GetUser("alice-id")
        if err != nil {
            t.Fatalf("GetUser 失败: %v", err)
        }
        if user.Name != "Alice" {
            t.Errorf("得到名字 %q; 期望 %q", user.Name, "Alice")
        }
    })
...
func TestHealthHandler(t *testing.T) {
    ...
    if resp.StatusCode != http.StatusOK {
        t.Errorf("得到状态 %d; 期望 %d", resp.StatusCode, http.StatusOK)
    }

    body, _ := io.ReadAll(resp.Body)
    if string(body) != "OK" {
        t.Errorf("得到响应体 %q; 期望 %q", body, "OK")
    }
}
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            ...
            if w.Code != tt.wantStatus {
                t.Errorf("得到状态 %d; 期望 %d", w.Code, tt.wantStatus)
            }

            if tt.wantBody != "" && w.Body.String() != tt.wantBody {
                t.Errorf("得到响应体 %q; 期望 %q", w.Body.String(), tt.wantBody)
            }
        })
    }
}
...
**记住**：测试即文档。它们展示了代码的使用方式。请清晰编写并保持更新。